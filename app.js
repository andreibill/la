...
const wrongMessages = [
  ...
  "Hmm, nu chiar. Mai ai o șansă (sau mai multe) 😄"
];
const correctMessage = "Corect! 💜";
...
const MLG_TOKENS = ['🔥','💯','😎','👌','🎉','💸','🕶️','⚡','🏆','🤯','WOW','MLG','420','GG','NO SCOPE','#REKT','HEADSHOT'];
function spawnMlgToken() {
  const el = document.createElement('div');
  el.className = 'mlg-token';
  el.textContent = MLG_TOKENS[Math.floor(Math.random() * MLG_TOKENS.length)];
...
function playVideoOverlay(srcs, loop) {
  ...
  // shrink each clip when several play side-by-side
  const maxW = list.length >= 3 ? 30 : list.length === 2 ? 44 : 80;
  ...
  list.forEach(src => {
    ...
    function render() {
      ...
      if (video.videoWidth) {
        // process at a capped resolution (CSS upscales) — much lighter on phones
        const scale = Math.min(1, 420 / video.videoWidth);
...
function pickAnswer(isCorrect, btn) {
  ...
  if (isCorrect) {
    ...
    document.getElementById('qFeedback').textContent = correctMessage;
    ...
    document.getElementById('qFeedback').textContent = msg;
  }
}
...
function playProgression() {
  ...
  document.getElementById('lastChord').textContent = 'Ascultă... 🎶';
  ...
  setTimeout(() => {
    ...
    document.getElementById('lastChord').textContent = 'Ce piesă era? Alege mai jos 👇';
  }, when * 1000 + 200);
}
...
function guessSong(index, btn) {
  if (index === CORRECT_SONG) {
    ...
    btn.classList.add('correct');
    document.getElementById('lastChord').textContent = 'Exact! 💜🎶';
    document.getElementById('pianoContinue').classList.remove('hidden');
  } else {
    btn.classList.add('wrong');
    btn.disabled = true;
    document.getElementById('lastChord').textContent = 'Nu chiar... mai ascultă o dată și încearcă 😊';
  }
}
...
function submitGuess() {
  ...
  if (wordleRow >= WORDLE_MAX) {
    ...
    document.getElementById('wordleMsg').textContent =
      `Indiciu: cuvântul e „${WORDLE_TARGET}". Mai scrie-l o dată 😊`;
...
function draw() {
  ...
  // gift target
  ctx.font = '64px serif';
  ctx.fillText('🎁', gift.x - 34, gift.y + 22);
...
const GIFT_MESSAGE = [
  "La mulți ani, Ilinca! 💜",
  "Sper că ți-a plăcut micul joc.",
  "Ești cel mai frumos om din viața mea. 💜",
  "Ly 💜"
];
...
function revealWheelCta() {
  ...
  host.innerHTML =
    `<div class="wheel-cta">
       <p class="wheel-intro">Stai... mai e ceva. 🎡</p>
       <button onclick="goToWheel()">Roata norocului 🎰</button>
     </div>`;
}
...
const WHEEL_SEGMENTS = [
  { label: '1000 lei',  emoji: '💰', color: '#e8632a', text: '#fff' }, // 0  <- TOP, REAL GIFT (rigged)
  { label: 'Nimic',     emoji: '😶', color: CREAM, text: BROWN },       // 1  <- near-miss neighbour
  { label: '600 lei',   emoji: '🪙', color: TAN,   text: BROWN },       // 2
  { label: 'Vacanță',   emoji: '🏝️', color: CREAM, text: BROWN },       // 3
  { label: 'Mașină',    emoji: '🚗', color: TAN,   text: BROWN },       // 4  (bottom)
  { label: 'Croazieră', emoji: '🛳️', color: CREAM, text: BROWN },       // 5
  { label: '300 lei',   emoji: '💵', color: TAN,   text: BROWN },       // 6
  { label: 'Inel',      emoji: '💍', color: CREAM, text: BROWN },       // 7
];
...
function buildWheel() {
  ...
  stage.innerHTML =
    `<div class="wheel-wrap">
       ...
       <div class="wheel" id="wheelDisk"
            style="background: ${lines}, conic-gradient(from ${WHEEL_FROM}deg, ${stops});"></div>
  ...
  WHEEL_SEGMENTS.forEach((s, i) => {
    ...
    lab.innerHTML = `<span><span class="we">${s.emoji}</span>${s.label}</span>`;
...
function showWheelResult() {
  ...
  if (res) {
    ...
    res.innerHTML =
      `💰 1000 DE LEI! 💰
       <span class="sub">Felicitări, ai câștigat marele premiu! 💜</span>`;
  }
  bigConfetti();
  const hub = document.getElementById('wheelHub');
  if (hub) { hub.classList.remove('spinning'); hub.textContent = '🎉'; }
}
...
async function checkGate() {
  ...
  if (h === GATE_HASH) {
    ...
    document.getElementById('gateMsg').textContent = 'Hmm, nu e bine. Mai încearcă 😏';
  }
}
function initGate() {
  let ok = false;
